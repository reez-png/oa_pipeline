...
def project_root() -> Path:
    """Project root (the directory containing the eight notebooks)."""
    return PROJECT_ROOT
...
def tiny_sample_frame() -> pd.DataFrame:
    ...
    Has the canonical Stage 1B columns (best fields + sources + booleans)
    plus deliberate variation row-by-row so threshold-based checks can
    exercise both directions:

      * row 0: clean PASS
      * row 1: clean PASS
      * row 2: salinity 50 (out of range) -> REVIEW range_flag
      * row 3: missing sample_id          -> FAIL missing_key
      * row 4: stage3 strict issue        -> FAIL stage3_strict_issue
    """
    return pd.DataFrame({
        # Identifiers
        ...
        "salinity":      [35.0, 35.0, 50.0, 35.0, 35.0],  # row 2 out of range
        ...
        # Stage 3 boolean carry-forwards
        ...
        "sample_month": pd.Series(
            ["2024-01", "2024-01", "2024-01", "2024-01", "2024-01"], dtype="string"
        ),
    })
